...
- Added 16 focused Worker tests, including an injected service-level verifier test proving failed delivery stops before settle/succeeded writes. The focused delivery suite and 21 related Worker regression tests pass.
...
- Added `000083_agent_v6_conversation.sql` to extend tenant-scoped V5 turns with progress, capability-reference, and result-reference JSON plus tenant-scoped decision idempotency. The V5/V6 migration chain was renumbered to 081/082/083 because deployed environments already use version 078 for the earlier V4 migration.
...
- validation completed locally: focused template/group frontend tests (`110` passed); workflow-core compiler tests (`11` passed); API/Worker builds. API and Worker database integration tests remain skipped because `DATABASE_URL` is not configured and Docker Desktop is unavailable locally. Run the documented local infra and database suite before staging.
...
## 2026-09-06 - TapNow-style Agent V5 Design Approval

- User confirmed a full Agent information-architecture replacement: fixed right-side window, one combined bottom Composer, history/new conversation, execution mode, model picker, canvas/upload attachments, Skill/App management, and no voice or usage-statistics buttons.
- Structured text formatting is a first-class protocol requirement: headings, lists, tables, choices, Brief, Skill/App, confirmation, progress and result-group blocks must render consistently in live turns and replay.
- Formal design: `docs/superpowers/specs/2026-09-06-tapnow-agent-v5-design.md`. Implementation remains gated on written-spec review; the existing V4 deployment is not an accepted final product.
- Implementation plan: `docs/superpowers/plans/2026-09-06-tapnow-agent-v5.md`. The plan covers the V5 block protocol, right-side window replacement, unified state/orchestrator, durable decisions, capabilities, result replay, and staging acceptance.
