...
- validation passed: `@aigc-flow/ai-gateway-core` full suite (`169` tests); `@aigc-flow/worker` full suite (`86` passed, `18` skipped because database integration is unavailable); focused frontend suite (`81` tests); API package build; root frontend build; focused API input-order test.
