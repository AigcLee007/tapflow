...
- added the required `pg.Pool` idle-client error listener after Supabase terminated a returned migration connection between committed migrations. Staging confirmed migrations `000042` and `000043` were safely recorded before the disconnect, while `000044` and `000045` remained pending; the listener now records the connection failure without crashing Node so the pool can discard the dead client and continue with a replacement connection.
