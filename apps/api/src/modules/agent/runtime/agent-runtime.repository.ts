...
export type AgentRuntimeStateInput = { sessionId: string; turnId: string; expectedStateVersion: number; expectedGraphRevision: number; graphRevision: number; phase: string; executionState: string; blocks: ConversationBlock[]; pendingDecision?: Record<string, unknown> | null; planJson?: Record<string, unknown>; contextSnapshot?: AgentContextSnapshot; status?: string };
...
async function saveStateLocked(client: PoolClient, ctx: AgentRuntimeContext, session: AgentRuntimeSession, input: AgentRuntimeStateInput): Promise<AgentRuntimeTurn> {
  ...
  const result = await client.query(`UPDATE agent_turns SET blocks_json=$4::jsonb,conversation_phase=$5,execution_state=$6,pending_decision_json=$7::jsonb,graph_revision=$8,status=COALESCE($9,status),plan_json=COALESCE($10::jsonb,plan_json),context_snapshot_json=COALESCE($11::jsonb,context_snapshot_json),state_version=state_version+1,updated_at=now() WHERE tenant_id=$1::uuid AND session_id=$2::uuid AND id=$3::uuid AND runtime_version='runtime' AND state_version=$12 AND graph_revision=$13 RETURNING ${turnColumns}`, [ctx.tenantId, input.sessionId, input.turnId, json(blocks), input.phase, input.executionState, json(input.pendingDecision ?? null), input.graphRevision, input.status ?? null, input.planJson === undefined ? null : json(input.planJson), input.contextSnapshot === undefined ? null : json(input.contextSnapshot), input.expectedStateVersion, input.expectedGraphRevision]);
