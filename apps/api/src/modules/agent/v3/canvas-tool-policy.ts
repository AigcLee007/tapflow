...
export function assertCanvasToolAllowed(request: { namespace: CanvasToolNamespace; toolName: string; tenantId: string; projectId: string; flowId: string; sessionId: string; expectedRevision: number }, context: CanvasToolPolicyContext) {
  ...
  if (!request.sessionId || request.sessionId !== context.sessionId) throw new CanvasToolPolicyError("session scope mismatch");
  if (!Number.isInteger(request.expectedRevision) || request.expectedRevision !== context.graphRevision) throw new CanvasToolPolicyError("stale graph revision");
