...
export class AgentV4RuntimeService {
  ...
  async startTurn(input: { sessionId: string; context: AgentV4RequestContext; body: unknown }) {
    ...
    const task = await store.create({ tenantId: input.context.tenantId, sessionId: input.sessionId, projectId: session.projectId, flowId: session.flowId, graphRevision: body.expectedGraphRevision, prompt: body.prompt, referenceAssetIds: body.referenceContext?.map((reference) => reference.assetId), idempotencyKey: body.idempotencyKey });
    const service = new AgentResponsesSessionService({ textRuntime: this.options.textRuntime, store, gateway: this.options.gateway ?? new AgentV4ToolGateway({ handlers: {
      ...
      "prompt_set.create": async ({ task, call }) => ({ ok: true, status: "preview_ready", items: createPromptItems(call.arguments.suitePlan as ReturnType<typeof createTaobaoSuitePlan>, call.arguments.visualBible as ReturnType<typeof createVisualBible>, task.referenceAssetIds ?? []) }),
