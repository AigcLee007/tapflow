...
export class AiModelCatalogService {
  ...
  async listRoutesForModel(
    ...
    return withTenantTransaction(context, async (client) => {
      ...
      const rows = await queryCatalogRoutes(client, context, {
...
async function queryCatalogRoutes(
  ...
  const result = await client.query<ModelRouteRecord>(
    `
      ...
      LEFT JOIN tenant_ai_plugin_installs AS route_install
        ON route_install.id = route.plugin_install_id
      LEFT JOIN LATERAL (
      ...
      WHERE (route.tenant_id = $1::uuid OR route.tenant_id IS NULL)
        ...
        AND (
          ...
          OR route.model_id IS NULL
          OR ($5::uuid IS NOT NULL AND route.model_id = $5::uuid)
          OR ($6::text IS NOT NULL AND model.model_key = $6::text)
        )
      ORDER BY
        route.route_key ASC,
...
function groupRoutesByModelKey(rows: ModelRouteRecord[]): Record<string, ModelCatalogRouteView[]> {
