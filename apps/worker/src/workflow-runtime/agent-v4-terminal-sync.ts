...
export function createDatabaseV4TerminalProjector(pool: Pool) {
  return async (input: { tenantId: string; workflowRunId: string }) => withTenantTransaction({ tenantId: input.tenantId, userId: null }, async (client) => {
    const binding = await client.query<{ task_id: string; session_id: string }>(`SELECT (wr.input_json->>'agentV4TaskId') AS task_id, at.session_id::text AS session_id FROM workflow_runs wr JOIN agent_tasks at ON at.id = (wr.input_json->>'agentV4TaskId')::uuid WHERE wr.tenant_id = $1::uuid AND wr.id = $2::uuid LIMIT 1`, [input.tenantId, input.workflowRunId]);
    const row = binding.rows[0]; if (!row?.task_id) return null;
