...
export function createDatabaseV4TerminalProjector(pool: Pool) {
  return async (input: { tenantId: string; workflowRunId: string }) => withTenantTransaction({ tenantId: input.tenantId, userId: null }, async (client) => {
    const binding = await client.query<{ task_id: string; session_id: string; workflow_status: string }>(`SELECT (wr.input_json->>'agentV4TaskId') AS task_id, at.session_id::text AS session_id, wr.status AS workflow_status FROM workflow_runs wr JOIN agent_tasks at ON at.id = (wr.input_json->>'agentV4TaskId')::uuid WHERE wr.tenant_id = $1::uuid AND wr.id = $2::uuid AND at.tenant_id = wr.tenant_id AND at.agent_version = 'v4' LIMIT 1`, [input.tenantId, input.workflowRunId]);
    ...
    const items = result.rows.map((item) => ({ itemId: item.node_id, status: item.status === "succeeded" ? "succeeded" : item.status === "cancelled" ? "cancelled" : "failed", ...(typeof item.output_json?.assetId === "string" ? { assetId: item.output_json.assetId } : {}), ...(typeof item.error_json?.code === "string" ? { errorCode: item.error_json.code } : {}) })) as V4TerminalItem[];
