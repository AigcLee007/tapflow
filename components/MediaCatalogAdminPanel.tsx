...
type CatalogMedia = 'image' | 'video';
type CatalogViewMode = 'matrix' | 'advanced';
...
const MatrixMetric = ({ label, value, tone = 'default' }: { label: string; value: React.ReactNode; tone?: 'default' | 'good' | 'warn' | 'bad' }) => {
  const toneClass =
    tone === 'good'
...
const routeHasCredential = (route: Pick<AdminImageRoute | AdminVideoRoute, 'apiKeyEnv' | 'hasApiKey'>) =>
  Boolean(String(route.apiKeyEnv || '').trim() || route.hasApiKey);
const routeCredentialLabel = (route: Pick<AdminImageRoute | AdminVideoRoute, 'apiKeyEnv' | 'hasApiKey'>) => {
  if (String(route.apiKeyEnv || '').trim()) return route.apiKeyEnv;
  if (route.hasApiKey) return '已保存密钥';
  return '缺少密钥';
};
const getRouteHealthIssues = (route: AdminImageRoute | AdminVideoRoute) => {
  const issues: string[] = [];
  if (route.isActive === false) issues.push('线路已停用');
  if (!routeHasCredential(route)) issues.push('缺少密钥');
  if (!String(route.baseUrl || '').trim()) issues.push('缺少 Base URL');
  if (!String(route.generatePath || '').trim()) issues.push('缺少提交路径');
  if (route.mode === 'async' && !String(route.taskPath || '').trim()) issues.push('异步线路缺少查询路径');
  if (roundNonNegativePoint(route.pointCost || 0, 0) <= 0) issues.push('点数未配置');
  const hasSizeModel = 'sizeOverrides' in route && hasImageRouteSizeOverrideModel(route);
  if (!String(route.upstreamModel || '').trim() && route.useRequestModel !== true && !hasSizeModel) {
    issues.push('缺少调用模型');
  }
  return issues;
};
const getModelHealthIssues = (
  model: AdminImageModel | AdminVideoModel,
  routes: Array<AdminImageRoute | AdminVideoRoute>,
) => {
  const issues: string[] = [];
  if (model.isActive === false) issues.push('模型已停用');
  if (!routes.length) issues.push('没有线路');
  if (routes.length && !routes.some((route) => route.isActive !== false)) issues.push('没有启用线路');
  if (routes.filter((route) => route.isDefaultRoute === true).length > 1) issues.push('默认线路冲突');
  if (routes.some((route) => route.isActive !== false && !routeHasCredential(route))) issues.push('启用线路缺少密钥');
  return issues;
};
...
const MediaCatalogAdminPanel: React.FC<MediaCatalogAdminPanelProps> = ({ session, dashboard, onRefreshDashboard }) => {
  ...
  const [viewMode, setViewMode] = useState<CatalogViewMode>('matrix');
  const [activeMedia, setActiveMedia] = useState<CatalogMedia>('image');
  ...
  const quickToggleImageModel = useCallback(async (model: AdminImageModel) => {
    setSaving(true);
    try {
      await updateAdminImageModel(model.id, { isActive: model.isActive === false });
      toast.success(model.isActive === false ? '图片模型已启用。' : '图片模型已停用。');
      await refreshAll();
    } catch (err) {
      toast.error((err as Error).message || '模型状态更新失败');
    } finally {
      setSaving(false);
    }
  }, [refreshAll, toast]);

  const quickToggleVideoModel = useCallback(async (model: AdminVideoModel) => {
    setSaving(true);
    try {
      await updateAdminVideoModel(model.id, { isActive: model.isActive === false });
      toast.success(model.isActive === false ? '视频模型已启用。' : '视频模型已停用。');
      await refreshAll();
    } catch (err) {
      toast.error((err as Error).message || '模型状态更新失败');
    } finally {
  ...
  const quickToggleImageRoute = useCallback(async (route: AdminImageRoute) => {
    setSaving(true);
    try {
      await updateAdminImageRoute(route.id, { isActive: route.isActive === false });
      toast.success(route.isActive === false ? '图片线路已启用。' : '图片线路已停用。');
      await refreshAll();
    } catch (err) {
      toast.error((err as Error).message || '线路状态更新失败');
    } finally {
      setSaving(false);
    }
  }, [refreshAll, toast]);

  const quickToggleVideoRoute = useCallback(async (route: AdminVideoRoute) => {
    setSaving(true);
    try {
      await updateAdminVideoRoute(route.id, { isActive: route.isActive === false });
      toast.success(route.isActive === false ? '视频线路已启用。' : '视频线路已停用。');
      await refreshAll();
    } catch (err) {
      toast.error((err as Error).message || '线路状态更新失败');
    } finally {
  ...
  const renderHealthPills = (issues: string[]) => {
    if (!issues.length) {
      return <SectionPill className="border-emerald-500/20 bg-emerald-500/15 text-emerald-100">正常</SectionPill>;
    }
    return (
      <>
        {issues.slice(0, 3).map((issue) => (
          <SectionPill key={issue} className="border-amber-500/25 bg-amber-500/15 text-amber-100">{issue}</SectionPill>
        ))}
        {issues.length > 3 ? <SectionPill className="border-amber-500/25 bg-amber-500/15 text-amber-100">+{issues.length - 3}</SectionPill> : null}
      </>
    );
  };
  ...
  const renderImageMatrix = () => (
    <div className="space-y-4">
      {filteredImageModels.length ? filteredImageModels.map((model) => {
        const routes = getImageRoutesForModel(model);
        const modelStat = imageModelStats.get(model.id) || imageModelStats.get(model.modelFamily);
        const modelIssues = getModelHealthIssues(model, routes);
        return (
          <div key={model.id} className="rounded-3xl border border-white/10 bg-black/20 p-4">
              ...
              <div className="flex min-w-0 items-start gap-3">
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <SectionPill className={model.isActive === false ? 'border-rose-500/30 bg-rose-500/15 text-rose-100' : 'border-emerald-500/20 bg-emerald-500/15 text-emerald-100'}>
                      {model.isActive === false ? '模型停用' : '模型启用'}
                    </SectionPill>
                    {model.isDefaultModel ? <SectionPill className="border-amber-500/30 bg-amber-500/15 text-amber-100">默认模型</SectionPill> : null}
                  </div>
                  <p className="mt-1 truncate text-xs text-cyan-200/75">{model.id} / {model.routeFamily}</p>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {renderHealthPills(modelIssues)}
                    {renderModelStats(modelStat)}
                  </div>
                </div>
              </div>
              <div className="flex flex-wrap gap-2 2xl:justify-end">
                <ActionButton onClick={() => void quickToggleImageModel(model)} icon={model.isActive === false ? <Power size={16} /> : <PowerOff size={16} />} tone={model.isActive === false ? 'success' : 'warning'} disabled={saving}>{model.isActive === false ? '启用模型' : '停用模型'}</ActionButton>
                ...
                <ActionButton onClick={() => openCopyRoutes('image', model.id)} icon={<Copy size={16} />}>复制线路</ActionButton>
              </div>
            </div>
              ...
              <table className="w-full min-w-[1040px] text-left text-sm">
                <thead className="bg-white/[0.04] text-xs text-gray-400">
                  <tr>
                    ...
                    <th className="px-3 py-3">线路</th>
                    <th className="px-3 py-3">状态</th>
                    <th className="px-3 py-3">调用配置</th>
                    <th className="px-3 py-3">点数</th>
                    <th className="px-3 py-3">密钥</th>
                    <th className="px-3 py-3">健康</th>
                    <th className="px-3 py-3">24h 表现</th>
                ...
                <tbody className="divide-y divide-white/10">
                  {routes.length ? routes.map((route) => {
                    ...
                    const routeIssues = getRouteHealthIssues(route);
                    return (
                      <tr key={route.id} className="bg-white/[0.015] align-top">
                        ...
                        <td className="px-3 py-3">
                          ...
                          <div className="mt-1 text-xs text-gray-500">{route.id} / {route.line}</div>
                        </td>
                        <td className="px-3 py-3">
                          <div className="flex flex-wrap gap-2">
                            <SectionPill className={route.isActive === false ? 'border-rose-500/30 bg-rose-500/15 text-rose-100' : 'border-emerald-500/20 bg-emerald-500/15 text-emerald-100'}>{route.isActive === false ? '停用' : '启用'}</SectionPill>
                            {route.isDefaultRoute ? <SectionPill className="border-amber-500/30 bg-amber-500/15 text-amber-100">默认</SectionPill> : null}
                            {route.isDefaultNanoBananaLine ? <SectionPill className="border-amber-500/30 bg-amber-500/15 text-amber-100">Nano</SectionPill> : null}
                          </div>
                        </td>
                        <td className="px-3 py-3">
                          <div className="max-w-[260px] space-y-1 text-xs text-gray-300">
                            <div>{route.transport} / {route.mode}</div>
                            <div className="truncate text-gray-500">{route.upstreamModel || (route.useRequestModel ? '使用模型请求名' : '未配置调用模型')}</div>
                            <div className="truncate text-gray-500">{route.baseUrl}</div>
                          </div>
                        </td>
                        <td className="px-3 py-3 whitespace-nowrap">{formatPoint(route.pointCost || 0)} 点</td>
                        <td className="px-3 py-3">
                          <div className="flex items-center gap-2 text-xs text-gray-300"><KeyRound size={14} />{routeCredentialLabel(route)}</div>
                        </td>
                        <td className="px-3 py-3"><div className="flex max-w-[220px] flex-wrap gap-2">{renderHealthPills(routeIssues)}</div></td>
                        <td className="px-3 py-3">
                          <div className="space-y-1 text-xs text-gray-300">
                            <div>{routeStat?.requestsLast24h || 0} 次</div>
                            <div className={Number(routeStat?.successRateLast24h || 0) >= 80 ? 'text-emerald-200' : 'text-amber-200'}>{formatRate(routeStat?.successRateLast24h)}</div>
                          </div>
                        </td>
                        <td className="px-3 py-3">
                          <div className="flex justify-end gap-2">
                            <ActionButton onClick={() => void quickToggleImageRoute(route)} icon={route.isActive === false ? <Power size={16} /> : <PowerOff size={16} />} tone={route.isActive === false ? 'success' : 'warning'} disabled={saving}>{route.isActive === false ? '启用' : '停用'}</ActionButton>
                    ...
                    <tr><td colSpan={9} className="px-4 py-6 text-center text-sm text-gray-400">当前模型还没有图片线路。</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        <div className="rounded-3xl border border-dashed border-white/10 bg-white/[0.02] px-4 py-10 text-center text-sm text-gray-400">
          {q ? '没有匹配的图片模型或线路。' : '还没有图片模型。'}
        </div>
      )}
    </div>
  );

  const renderVideoMatrix = () => (
    <div className="space-y-4">
      {filteredVideoModels.length ? filteredVideoModels.map((model) => {
        const routes = getVideoRoutesForModel(model);
        const modelStat = videoModelStats.get(model.id) || videoModelStats.get(model.modelFamily);
        const modelIssues = getModelHealthIssues(model, routes);
        return (
          <div key={model.id} className="rounded-3xl border border-white/10 bg-black/20 p-4">
            <div className="flex flex-col gap-4 2xl:flex-row 2xl:items-start 2xl:justify-between">
              <div className="flex min-w-0 items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 truncate text-xs text-fuchsia-200/75">{model.id} / {model.routeFamily}</p>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {renderHealthPills(modelIssues)}
                    {renderModelStats(modelStat)}
                  </div>
              ...
              <div className="flex flex-wrap gap-2 2xl:justify-end">
                <ActionButton onClick={() => void quickToggleVideoModel(model)} icon={model.isActive === false ? <Power size={16} /> : <PowerOff size={16} />} tone={model.isActive === false ? 'success' : 'warning'} disabled={saving}>{model.isActive === false ? '启用模型' : '停用模型'}</ActionButton>
                ...
                <ActionButton onClick={() => openCopyRoutes('video', model.id)} icon={<Copy size={16} />}>复制线路</ActionButton>
              </div>
            </div>
            <div className="mt-4 overflow-x-auto rounded-2xl border border-white/10">
              <table className="w-full min-w-[1040px] text-left text-sm">
                  ...
                  {routes.length ? routes.map((route) => {
                    ...
                    return (
                      <tr key={route.id} className="bg-white/[0.015] align-top">
                        ...
                        <td className="px-3 py-3">
                          <div className="flex justify-end gap-2">
                            <ActionButton onClick={() => void quickToggleVideoRoute(route)} icon={route.isActive === false ? <Power size={16} /> : <PowerOff size={16} />} tone={route.isActive === false ? 'success' : 'warning'} disabled={saving}>{route.isActive === false ? '启用' : '停用'}</ActionButton>
                    ...
                    <tr><td colSpan={9} className="px-4 py-6 text-center text-sm text-gray-400">当前模型还没有视频线路。</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        <div className="rounded-3xl border border-dashed border-white/10 bg-white/[0.02] px-4 py-10 text-center text-sm text-gray-400">
          {q ? '没有匹配的视频模型或线路。' : '还没有视频模型。'}
        </div>
      )}
    </div>
  );

  const renderMatrixWorkbench = () => {
    const isImage = activeMedia === 'image';
    const models = isImage ? filteredImageModels : filteredVideoModels;
    const routes = isImage
      ? filteredImageModels.flatMap((model) => getImageRoutesForModel(model))
      : filteredVideoModels.flatMap((model) => getVideoRoutesForModel(model));
    const riskModels = isImage
      ? filteredImageModels.filter((model) => getModelHealthIssues(model, getImageRoutesForModel(model)).length)
      : filteredVideoModels.filter((model) => getModelHealthIssues(model, getVideoRoutesForModel(model)).length);
    const riskRoutes = routes.filter((route) => getRouteHealthIssues(route).length);
    const activeModels = models.filter((model) => model.isActive !== false).length;
    const activeRoutes = routes.filter((route) => route.isActive !== false).length;
    const mediaTone = isImage ? 'text-cyan-200' : 'text-fuchsia-200';
        ...
        <div className="flex flex-col gap-4 2xl:flex-row 2xl:items-start 2xl:justify-between">
          <div>
            <div className="flex items-center gap-2">
              <Table2 size={18} className={mediaTone} />
              <h3 className="text-lg font-semibold">模型线路矩阵</h3>
            </div>
            <p className="mt-1 text-sm text-gray-400">按模型聚合线路，直接查看启停、默认、价格、密钥、接口配置和最近成功率。</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <ActionButton onClick={() => setActiveMedia('image')} tone={activeMedia === 'image' ? 'primary' : 'default'} icon={<Boxes size={16} />}>图片</ActionButton>
            <ActionButton onClick={() => setActiveMedia('video')} tone={activeMedia === 'video' ? 'primary' : 'default'} icon={<Clapperboard size={16} />}>视频</ActionButton>
            <ActionButton onClick={() => (isImage ? openCreateImageModel() : openCreateVideoModel())} tone="primary" icon={<Plus size={16} />}>新增模型</ActionButton>
            <ActionButton onClick={() => selectVisibleItems(activeMedia, 'models')}>全选模型</ActionButton>
            <ActionButton onClick={() => selectVisibleItems(activeMedia, 'routes')}>全选线路</ActionButton>
            <ActionButton onClick={() => void bulkToggle(activeMedia, true)} icon={<Power size={16} />} tone="success">启用选中</ActionButton>
            <ActionButton onClick={() => void bulkToggle(activeMedia, false)} icon={<PowerOff size={16} />} tone="warning">停用选中</ActionButton>
          </div>
        </div>

        <div className="mt-5 grid gap-3 md:grid-cols-4">
          <MatrixMetric label="启用模型" value={`${activeModels}/${models.length}`} tone={activeModels === models.length ? 'good' : 'warn'} />
          <MatrixMetric label="启用线路" value={`${activeRoutes}/${routes.length}`} tone={activeRoutes === routes.length ? 'good' : 'warn'} />
          <MatrixMetric label="风险模型" value={riskModels.length} tone={riskModels.length ? 'bad' : 'good'} />
          <MatrixMetric label="风险线路" value={riskRoutes.length} tone={riskRoutes.length ? 'bad' : 'good'} />
        </div>

        {riskRoutes.length || riskModels.length ? (
          <div className="mt-4 flex items-start gap-3 rounded-2xl border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
            ...
            <div>当前筛选下有 {riskModels.length} 个模型、{riskRoutes.length} 条线路需要检查，优先处理缺少密钥、没有启用线路和默认线路冲突。</div>
          </div>
        ) : null}

        <div className="mt-5">
          {isImage ? renderImageMatrix() : renderVideoMatrix()}
        </div>
      </section>
    );
  };
    ...
    <div className="space-y-5 text-white">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <div className="flex rounded-2xl border border-white/10 bg-black/20 p-1">
              <button type="button" onClick={() => setViewMode('matrix')} className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm transition ${viewMode === 'matrix' ? 'bg-cyan-500/20 text-cyan-100' : 'text-gray-400 hover:text-white'}`}><Eye size={16} />矩阵视图</button>
              <button type="button" onClick={() => setViewMode('advanced')} className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm transition ${viewMode === 'advanced' ? 'bg-cyan-500/20 text-cyan-100' : 'text-gray-400 hover:text-white'}`}><Settings2 size={16} />高级列表</button>
            </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
        <div className="space-y-5">
          {viewMode === 'matrix' ? renderMatrixWorkbench() : (
            <>
          <section className="rounded-3xl border border-white/10 bg-[#071221]/85 p-5 shadow-[0_16px_60px_rgba(0,0,0,0.28)]">
