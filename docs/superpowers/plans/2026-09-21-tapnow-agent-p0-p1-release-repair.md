...
本文件是后续 Agent P0/P1 修复的执行依据。每个任务必须先写失败测试、运行确认红测、实现最小修复、运行确认绿测、执行构建与 `git diff --check`，然后独立提交并通过阶段 Gate。P0-A 至 P0-D 任一 Gate 未通过，不得进入 staging；两个 Agent runtime flags 在全部验收前保持关闭。

### P0-A：结果 block、group 与 pending decision 绑定

- 文件：`apps/api/src/modules/agent/runtime/agent-runtime.service.ts`、`agent-runtime.repository.ts`、`agent-runtime.schemas.ts`。
- 修复：生成真实唯一 `blockId`，result group/block/pending decision 共用该 ID；事务内校验 tenant/session/turn/run/group/block；使用 `stateVersion` CAS 和稳定幂等 key；历史格式只读兼容。
- 失败测试：真实 blockId 绑定、上下文不一致拒绝、重复 approve 幂等、并发 approve 返回 409、禁止新写入固定 `results`。
- Gate：真实 PostgreSQL 中生成、approve、place、重复 approve、并发 approve 均通过，不出现 `AGENT_PENDING_DECISION_INVALID`。

### P0-B：状态机、retry/revise、cancel、auto 启动失败

- 文件：`apps/api/src/modules/agent/runtime/agent-runtime.service.ts`、`agent-runtime.repository.ts`、`apps/api/src/modules/workflow-runs/workflow-runs.service.ts`、`apps/worker/src/workflow-runtime/service.ts` 及 billing reserve/refund/release 实现。
- 状态：`queued → planning → awaiting_input → awaiting_approval → running → verifying → succeeded | failed | cancelled`。终态不得任意互转，cancelled 不得映射为 failed。
- 失败测试：failed retry 新建 `runId/attemptNo/retryOfRunId`；revise 新 plan revision；旧 failed run 不可 claim；cancel 幂等；enqueue 失败不永久 queued；过期 lease 可恢复、活跃 lease 不可重复 claim。
- Gate：API、worker、billing 状态一致；重复 job 不重复执行或退款。

### P0-C：delivery validation 先于 billing settle

- 文件：`apps/worker/src/workflow-runtime/service.ts`、`agent-v6-delivery.ts`、共享 delivery validator、`apps/api/src/modules/agent/runtime/agent-execution-adapter.ts`、workflow/billing service。
- 顺序：`reserve → execute → persist outputs → validate delivery → settle`。
- 失败测试：空文本、缺 `assetId`、跨 tenant asset、缺首帧/尾帧、首尾帧 metadata 不完整、节点未完成均不得 settle；失败只幂等 refund/release；成功只 settle 一次。
- Gate：billing ledger 只有一个终态，不存在 settle 后才发现交付失败。

### P0-D：run/step/result lineage 隔离

- 文件：result schema/repository、Agent runtime service/repository、worker delivery validator、`packages/db/migrations/` additive migration。
- 修复：结果携带 `runId`、`stepId`、`resultId`、`attemptNo`，引用旧结果额外携带 `sourceRunId`；查询、去重和验证只按 tenant + run + step + kind；删除 `kind + label` 唯一 fallback。
- 失败测试：第一次 run 结果不能满足第二次 run 缺失步骤；重复 label 不跨 run 匹配；并发 run 不交叉复用。
- Gate：P0-A 至 P0-D 全部通过后才允许 staging。

### P1-E：result action 状态和 reference 持久化

- 文件：Agent runtime service/repository/schemas、session context persistence、`src/flowCanvas/agent/CanvasAgentPanel.tsx`、`src/flowCanvas/agent/runtime/useAgentRuntime.ts`。
- 修复：action 前校验 tenant/session/turn/run/group/status；`place` 只接受单数 `resultId`，多 ID 返回 `AGENT_RESULT_ACTION_SINGLE`；reference 与 context 同事务持久化 `assetId/resultId/sourceRunId/contextVersion`；planner 从服务端 context 读取。
- 失败测试：pending/failed/unknown action 拒绝；跨 tenant/run 拒绝；设为参考后刷新、重开历史 session、继续下一轮仍携带 reference。

### P1-F：events、replay、SSE 统一协议

- 文件：`apps/api/src/modules/agent/agent.routes.ts`、event repository/service、`src/flowCanvas/agent/v6/replay/ReplayState.ts`、`agentV6Api.ts`、canonical runtime API/reducer。
- 协议：`{ events, lastSeq, replayCursor, resyncRequired }`；`eventJson` 为 canonical 字段，旧 `event` 仅兼容读取。
- 失败测试：顺序、重复 seq、缺号 resync、旧 cursor、Last-Event-ID、heartbeat、abort、断线重连；stream 不能读一次立即关闭。

### P1-G：错误映射、兼容开关、输入契约、model lock、history 脱敏

- 映射：403 权限、404 不存在、409 CAS/decision/state 冲突、400/422 非法参数、503 runtime disabled；错误体固定 `{ code, message, requestId }`。
- 修复：V3/V5/V6/legacy 统一受 flags 控制；空画布允许 `projectId/flowId = null`；非空资源检查 tenant ownership；planner 锁定 `modelKey`；history/events/node data/logs 不暴露 routeKey、credential、内部 node ID。
- 首尾帧契约：只要求两张图片和视频提示词时不创建 video step；明确要求生成视频时重新报价并确认；主体、变化、比例、时长和首尾双引用齐全才可执行。

### P1-H：canonical 前端默认入口与可见恢复错误

- 文件：`src/flowCanvas/agent/runtime/agentRuntimeApi.ts`、`agentEventReducer.ts`、`useAgentRuntime.ts`、`CanvasAgentPanel.tsx`；V5/V6 仅作为 flags 控制的兼容适配器。
- 失败测试：默认 import graph 不依赖 V5/V6 权威状态；历史切换同步 URL；刷新恢复 references/context；上传、history、place refresh 失败有可见错误和重试；首轮提交后 model lock。

## 依赖关系

`P0-A → P0-C`；`P0-A → P1-E`；`P0-B → P0-D`；`P0-D → P0-C`；`P0-D → P1-E`；P1-E/F/G 可在 canonical 类型稳定后并行；P1-H 必须在 API canonical contract 稳定后执行。

## 发布门禁与真实验证

必须执行 `npm run build`、`npm test`、API/worker/db/ai-gateway focused tests、`git diff --check`，以及真实 PostgreSQL、Redis/BullMQ、S3、Billing、AI Gateway 和 authenticated browser golden flow。没有命令输出的项目保持 `UNRESOLVED`。staging 使用 `docker-compose.staging.yml`：build → stop worker → `node packages/db/dist/cli.js` migration → start Redis/API/worker/frontend → 检查 ps/logs。

回滚保留上一可部署 commit/image；先停 worker，再回退 API/worker/frontend；优先关闭 `AGENT_RUNTIME_ENABLED` 和 `AGENT_RUNTIME_COMPAT_ENABLED`；异常 route 设为 inactive；数据库只做 additive migration，保留旧字段和 event decoder，billing ledger 不通过删除记录回滚。

## 回滚方案

- 保留上一可部署 commit/image。
- 回滚时先停止 worker，再回退 API/worker/frontend，并按 Docker Compose v2 顺序重新启动。
- 优先关闭 `AGENT_RUNTIME_ENABLED` 和 `AGENT_RUNTIME_COMPAT_ENABLED`；异常 AI route 设为 `inactive`。
- 数据库只做 additive migration；保留旧字段和 event decoder 至少一个发布周期。
- billing ledger、历史结果和历史 route 不通过删除记录回滚。
- 回滚后重新执行登录、history、健康检查和 billing ledger consistency smoke test，并记录命令输出。
