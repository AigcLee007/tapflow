...
输入“我要生成两张图，用来测试一个首尾帧视频的生成，需要一个首帧图片和一个尾帧图片，同时需要首尾帧视频的提示词”时，Agent 必须先理解交付物并只询问缺失的主体、变化、比例和时长；随后展示真实图片模型、数量、费用和写入范围，等待确认。确认后只生成首帧、尾帧和视频提示词，图片进入资产库并通过结果组展示；视频本体不因提示词请求而自动生成。用户后续明确确认视频生成时，首尾帧资产通过稳定 assetId/角色引用进入独立视频任务。普通海报、商品图、文案和视频请求必须按各自目标规划，不得进入固定首尾帧或儿童玩具流程。
