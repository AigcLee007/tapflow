-- Exclude grants held by an in-flight refund from new wallet reserves.
...
BEGIN
  ...
  END IF;
  EXECUTE format('GRANT EXECUTE ON FUNCTION app.wallet_reserve(uuid, uuid, numeric, text, uuid, uuid, jsonb) TO %I', runtime_role);
END;
$$;
