  ...
  return `(async (page) => {
    ...
    for (const [name, viewport, screenshot] of cases) {
      ...
      if (name === "desktop") {
        ...
        await p.getByRole("option", { name: "Unconnected video" }).click();
        await p.waitForFunction(() => window.traySmokeState().edges.some((edge) => edge.source === "upstream-video"));
        await p.waitForFunction(() => window.traySmokeState().nodes.find((node) => node.id === "tray-video").data.generationPrompt?.includes("@视频1"));
        canvasConnected = true;
        await prompt.click();
        await prompt.press("@");
        await p.getByRole("listbox").waitFor({ state: "visible" });
        await p.getByRole("option", { name: "Library image" }).click();
        await p.waitForFunction(() => window.traySmokeState().nodes.find((node) => node.id === "tray-video").data.inputOrder?.includes("asset:asset-library"));
        await p.waitForFunction(() => window.traySmokeState().nodes.find((node) => node.id === "tray-video").data.generationPrompt?.includes("@图片2"));
        assetAdded = true;
        mentions = true;
        reordered = true;
      }      if (name === "tablet") {
