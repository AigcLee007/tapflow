...
import {getMediaMentionLabel} from '/src/flowCanvas/mentions/mediaReferenceIdentity.ts';
...
const imageNode={id:'upstream-image',type:'image',position:{x:10,y:180},data:{kind:'image',title:'Reference image',assetId:'asset-smoke-image',thumbnailUrl:'/logo.png',updatedAt:1}};
...
function TrayVideoNode({id,data,selected}){const index=useFlowCanvasStore(s=>s.graphIndex.upstreamInputRefsByNodeId[id]||[]);const remove=useFlowCanvasStore(s=>s.removeNodeInput);const removeText=useFlowCanvasStore(s=>s.removeTextNodeInputs);const reorder=useFlowCanvasStore(s=>s.reorderNodeInputs);const update=useFlowCanvasStore(s=>s.updateNodeData);const connect=useFlowCanvasStore(s=>s.connectNodes);const items=useMemo(()=>resolveCanvasInputItems({inputOrder:data.inputOrder,seeds:index}),[data.inputOrder,index]);const connected=index.map(x=>({inputKey:x.inputKey,kind:x.kind,title:x.title,sourceNodeId:x.sourceNodeId,assetId:x.assetId,thumbnailUrl:x.thumbnailUrl,mentionLabel:x.mentionLabel}));const candidates=useMemo(()=>buildMediaMentionCandidates({allowedKinds:new Set(['image','video']),assets:[{assetId:'asset-library',kind:'image',title:'Library image',thumbnailUrl:'/logo.png'}],canvas:[{nodeId:'upstream-video',kind:'video',title:'Unconnected video',thumbnailUrl:'/logo.png'}],connected,currentNodeId:id,recentAssetIds:[]}),[id,index]);const activate=(candidate)=>{if(candidate.activation.type==='canvas'){connect(candidate.activation.nodeId,id);return {inputKey:'upstream:'+candidate.activation.nodeId,kind:candidate.mediaKind,label:candidate.mentionLabel||getMediaMentionLabel(candidate.mediaKind,1)};}if(candidate.activation.type==='asset'){const current=useFlowCanvasStore.getState().nodes.find(node=>node.id===id)?.data;update({inputOrder:[...(current?.inputOrder||[]),'asset:'+candidate.activation.assetId],referenceAssetItemIds:['asset-library']});return {inputKey:'asset:'+candidate.activation.assetId,kind:candidate.mediaKind,label:candidate.mentionLabel||getMediaMentionLabel(candidate.mediaKind,2)};}return {inputKey:candidate.activation.inputKey,kind:candidate.mediaKind,label:candidate.mentionLabel};};window.traySmokeCandidates=candidates;window.traySmokeActivate=activate;return React.createElement('div',{style:{width:600},'data-testid':'tray-video-node'},React.createElement(VideoNodeComposer,{allowMediaAdd:false,catalog,data,generating:false,inputItems:items,mentionCandidates:candidates,nodeId:id,onActivateMentionCandidate:activate,onGenerate:()=>{},onRemoveInput:key=>remove(id,key),onRemoveAllText:()=>removeText(id),onReorderInputs:keys=>reorder(id,keys),onUpdate:patch=>update(id,patch),selected}));}
  ...
  return `(async (page) => {
    ...
    for (const [name, viewport, screenshot] of cases) {
      ...
      if (name === "desktop") {
        await prompt.click();
        await prompt.press("@");
        await p.getByRole("listbox").waitFor({ state: "visible" });
        if (await p.getByRole("option", { name: /Script|Direction/ }).count()) throw new Error("text candidate leaked");
        await p.keyboard.press("ArrowDown");\n        await p.keyboard.press("Enter");
        await p.waitForFunction(() => window.traySmokeState().edges.some((edge) => edge.source === "upstream-video")).catch(async () => { throw new Error("edge wait: " + JSON.stringify(await p.evaluate(() => window.traySmokeState()))); });
        await p.waitForFunction(() => window.traySmokeState().nodes.find((node) => node.id === "tray-video").data.generationPrompt?.includes("@视频1")).catch(async () => { throw new Error("video prompt wait: " + JSON.stringify(await p.evaluate(() => window.traySmokeState()))); });
        canvasConnected = true;
        await prompt.click();
        await prompt.press("@");
        await p.getByRole("listbox").waitFor({ state: "visible" });
        await p.keyboard.press("ArrowDown");\n        await p.keyboard.press("ArrowDown");\n        await p.keyboard.press("Enter");
        await p.waitForFunction(() => window.traySmokeState().nodes.find((node) => node.id === "tray-video").data.mediaMentionBindings?.some((binding) => binding.inputKey === "asset:asset-library")).catch(async () => { throw new Error("asset binding wait: " + JSON.stringify(await p.evaluate(() => window.traySmokeState()))); });
        await p.waitForFunction(() => window.traySmokeState().nodes.find((node) => node.id === "tray-video").data.generationPrompt?.includes("@图片2")).catch(async () => { throw new Error("image prompt wait: " + JSON.stringify(await p.evaluate(() => window.traySmokeState()))); });
        assetAdded = true;
        mentions = true;
        reordered = true;
      }      if (name === "tablet") {
