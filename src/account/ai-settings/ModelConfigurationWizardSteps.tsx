...
export function ModelConfigurationWizardSteps({ connections, credentials, onChange, plugins, providers, state, step }: WizardStepProps) {
  ...
  if (step === "routeCredential") {
    const providerKey = state.modelSource.type === "builtin" ? state.modelSource.provider?.key : state.modelSource.type === "custom" ? state.modelSource.custom.provider.key : "";
    const providerId = state.saved?.credential.providerId ?? providers.find((provider) => provider.key === providerKey)?.id;
    const safeCredentials = credentials.filter((credential) => !providerId || credential.providerId === providerId);
...
function Choice({ label, onChange, options, value }: { label: string; onChange: (value: string) => void; options: Array<{ label: string; value: string }>; value: string }) { return <div><label className="mb-1 block text-xs font-bold text-slate-300">{label}</label><MenuSelect fullWidth label={label} onChange={onChange} options={options.length ? options : [{ label: "暂无可用项", value: "" }]} value={value} /></div>; }
