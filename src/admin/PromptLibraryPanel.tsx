...
export function PromptLibraryPanel() {
  ...
  const movePrompt = async (id: string, direction: -1 | 1) => {
    ...
    const previous = items; const next = [...items]; [next[index], next[target]] = [next[target]!, next[index]!]; setItems(next);
  ...
  const dropPrompt = (targetId: string) => { const sourceId = dragId.current; dragId.current = null; if (!sourceId || sourceId === targetId) return; const from = items.findIndex((item) => item.id === sourceId); const to = items.findIndex((item) => item.id === targetId); if (from < 0 || to < 0) return; const next = [...items]; const [moved] = next.splice(from, 1); next.splice(to, 0, moved!); const previous = items; setItems(next); void reorderAdminPrompts({ promptIds: next.map((item) => item.id) }).then(setItems).catch((cause) => { setItems(previous); setError(cause instanceof Error ? cause.message : "排序保存失败"); }); };
