...
export function TemplateAdminEditorPage({ templateId }: { templateId: string }) {
  ...
  const addInputMarker = () => {
    ...
    const next = { id: inputDraft.id.trim(), label: inputDraft.label.trim(), required: inputDraft.required, type: inputDraft.type, target: { nodeId, fieldPath }, ...(inputDraft.type === 'enum' ? { options: enumOptions } : {}) } as FlowTemplateInputDefinition;
