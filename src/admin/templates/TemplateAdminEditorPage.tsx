...
export function TemplateAdminEditorPage({ templateId }: { templateId: string }) {
  ...
  const mutate = async (action: "archive" | "publish" | "save" | "testing") => {
    ...
    try {
      ...
      else if (action === "testing") next = await markAdminFlowTemplateTesting(template.id);
