...
export function RegisterPanel({ onModeChange, onPendingChange }: AuthPanelProps) {
  ...
  const [consented, setConsented] = useState(false); const [manifest, setManifest] = useState<LegalManifest | null>(null);
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); if (!consented) { setError(consentError); return; } if (!manifest) { setError("协议内容加载失败，请刷新后重试。"); return; } const request = requestGuard.begin(); setPending(true); setError(null); try { const result = await register({ consent: { privacyVersion: manifest.privacy.version, termsVersion: manifest.terms.version }, displayName: displayName.trim() || undefined, email, password }); if (!requestGuard.isCurrent(request)) return; if (result.status === "verification_required") { setChallenge(result); return; } navigate(getSafeReturnTo()); } catch (caught) { if (requestGuard.isCurrent(request)) setError(caught instanceof Error ? caught.message : "创建账号失败，请稍后重试。"); } finally { if (requestGuard.isCurrent(request)) setPending(false); } };
  ...
  return <form onSubmit={submit}>{challenge ? <><AuthErrorMessage message={error} /><EmailVerificationStep challenge={challenge} error={error} onBack={() => { requestGuard.cancel(); setChallenge(null); setError(null); setPending(false); }} onResend={resend} onVerify={verify} submitting={pending} /></> : <><AuthErrorMessage message={error} /><AuthField autoComplete="name" label="昵称" onChange={setDisplayName} value={displayName} /><AuthField autoComplete="email" label="邮箱" onChange={setEmail} required type="email" value={email} /><AuthField autoComplete="new-password" label="密码" minLength={8} onChange={setPassword} required type="password" value={password} /><LegalConsentControl checked={consented} error={error === consentError ? error : null} onChange={(value) => { setConsented(value); if (value && error === consentError) setError(null); }} /><AuthPrimaryButton disabled={pending || !manifest}>{pending ? "创建中..." : "创建账号"}</AuthPrimaryButton><AuthSecondaryButton onClick={() => mode("login")}>返回登录</AuthSecondaryButton></>}</form>;
}
