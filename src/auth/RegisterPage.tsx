...
export function RegisterPanel({ onModeChange, onPendingChange }: AuthPanelProps) {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); const request = requestGuard.begin(); setPending(true); setError(null); try { const result = await register({ displayName: displayName.trim() || undefined, email, password }); if (!requestGuard.isCurrent(request)) return; if (result.status === "verification_required") { setChallenge(result); return; } navigate(getSafeReturnTo()); } catch (caught) { if (requestGuard.isCurrent(request)) setError(caught instanceof Error ? caught.message : "Unable to create your account."); } finally { if (requestGuard.isCurrent(request)) setPending(false); } };
  ...
  return <form onSubmit={submit}>{challenge ? <><AuthErrorMessage message={error} /><EmailVerificationStep challenge={challenge} error={error} onBack={() => { requestGuard.cancel(); setChallenge(null); setError(null); setPending(false); }} onResend={resend} onVerify={verify} submitting={pending} /></> : <><AuthErrorMessage message={error} /><AuthField autoComplete="name" label="Display name" onChange={setDisplayName} value={displayName} /><AuthField autoComplete="email" label="Email" onChange={setEmail} required type="email" value={email} /><AuthField autoComplete="new-password" label="Password" minLength={8} onChange={setPassword} required type="password" value={password} /><AuthPrimaryButton disabled={pending}>{pending ? "Creating account..." : "Create account"}</AuthPrimaryButton><AuthSecondaryButton onClick={() => mode("login")}>Back to sign in</AuthSecondaryButton></>}</form>;
}
