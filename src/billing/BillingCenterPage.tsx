...
export function BillingCenterPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    } catch (cause) { if (id === requestRef.current) setError(cause instanceof Error ? cause.message : "Unable to load wallet"); }
  }, [authenticated, user]);
  ...
  return <div className="-mx-6 -my-9 min-h-[calc(100vh-80px)] bg-[#0b0b0d] px-6 py-8 sm:px-8">
    <main className="mx-auto max-w-[1440px]">
      <header><h1 className="text-2xl font-semibold text-white">Personal wallet</h1><p className="mt-2 text-sm text-slate-400">Your credits are shared across every workspace you can access.</p></header>
