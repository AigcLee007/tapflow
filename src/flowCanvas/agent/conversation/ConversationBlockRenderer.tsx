...
function Block({ block, onAction }: { block: ConversationBlock; onAction?: (action: ConversationBlockAction) => void }) {
  ...
  if (block.type === "choices" || block.type === "choice_grid") return <section style={cardStyle}>{title && <strong style={headingStyle}>{title}</strong>}<OptionList options={block.options} multiple={block.multiple} onAction={onAction} /></section>;
  ...
  return <section style={cardStyle}><strong style={headingStyle}>{title ?? "生成结果"}</strong><div style={{ display: "grid", gap: 6 }}>{block.results.map((result) => <div key={result.id} style={{ display: "grid", gap: 5 }}><button style={buttonStyle} onClick={() => onAction?.({ type: "result", id: result.id })}>{result.label}<small>{result.status ?? "ready"}</small></button><button style={{ ...buttonStyle, minHeight: 30 }} onClick={() => onAction?.({ type: "result", id: result.id, value: "refine" })}>继续编辑</button></div>)}</div></section>;
}
