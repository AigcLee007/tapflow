...
export function CanvasAgentV4TaskPanel({ task, onSubmit, onApprove, onCancel, onRetry, onUndo }: { task?: CanvasAgentV4Task; onSubmit?: (prompt: string) => void; onApprove?: () => void; onCancel?: () => void; onRetry?: (itemId: string) => void; onUndo?: (revision: number) => void }) {
  ...
  const dependencyGraph = latest<Array<{ from: string; to: string }>>(task, "dependencyGraph") ?? suitePlan?.pages.flatMap((page) => page.dependsOn.map((from) => ({ from, to: page.pageKey })));
  const completed = task.generationItems?.filter((item) => item.status === "succeeded").length ?? 0;
  return <aside aria-label="Canvas Agent V4 task panel"><h2>Canvas Agent V4</h2><p>状态：{task.status}</p><p>事件：{task.events.length}</p><form onSubmit={(event) => { event.preventDefault(); if (prompt.trim()) { onSubmit?.(prompt.trim()); setPrompt(""); } }}><input aria-label="V4 prompt" value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="继续描述或修复某一页" /><button type="submit">发送</button></form>{task.status === "waiting_for_approval" && <button type="button" onClick={onApprove}>批准执行</button>}{!['succeeded', 'failed', 'cancelled'].includes(task.status) && <button type="button" onClick={onCancel}>取消任务</button>}
    ...
    {visualBible && <section aria-label="Visual Bible"><h3>视觉规范</h3><p>{visualBible.productLock}</p><p>配色：{visualBible.palette.join("、")}</p><p>{visualBible.composition}；{visualBible.lighting}</p><p>{visualBible.background}；字体：{visualBible.typography}</p><p>禁用：{visualBible.prohibitions.join("；")}</p></section>}
    ...
    {task.generationItems && task.generationItems.length > 0 && <section aria-label="生成进度"><h3>生成进度 {completed}/{task.generationItems.length}</h3>{task.generationItems.map((item) => <div key={item.itemId}>{item.itemId}：{item.status}{item.assetId ? ` · ${item.assetId}` : ""}</div>)}</section>}{succeeded.length > 0 && <section aria-label="已生成结果"><h3>已生成 {succeeded.length} 项</h3>{succeeded.map((item) => <div key={item.itemId}>{item.itemId}{item.assetId ? ` · ${item.assetId}` : ""}</div>)}</section>}{failed.map((item) => <button key={item.itemId} type="button" onClick={() => onRetry?.(item.itemId)}>重试 {item.itemId}</button>)}{appliedRevision !== undefined && onUndo && <button type="button" onClick={() => onUndo(appliedRevision)}>撤销交付</button>}</aside>;
}
