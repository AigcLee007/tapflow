...
export type AgentComposerProps = {
  ...
  onLayerChange?: (layer: "capability" | "mode" | "model" | null) => void;
  ...
  return (
    <footer className="agent-v6-composer agent-v6-composer-fixed" data-testid="agent-v6-composer">
      ...
      <div className="agent-v6-composer-toolbar">
        <AgentCapabilityMenu onOpenChange={(open) => onLayerChange?.(open ? "capability" : null)} onSelect={onCapability} />
        <div className="agent-v6-mode" data-composer-slot="mode"><MenuSelect label="执行模式" value={mode} options={[{ label: "Agent 自动执行", value: "auto" }, { label: "用户确认", value: "manual_confirmation" }]} onChange={(value) => onModeChange?.(value as AgentExecutionMode)} onOpenChange={(open) => onLayerChange?.(open ? "mode" : null)} size="compact" /></div>
        ...
        <div className="agent-v6-model" data-composer-slot="model"><MenuSelect label="模型" value={model} options={[...modelOptions]} onChange={(value) => onModelChange?.(value)} onOpenChange={(open) => onLayerChange?.(open ? "model" : null)} size="compact" /></div>
