...
export function dateLabel(date: string, now: Date) {
...
export function AgentHistory({ items, loading = false, now, onSelect, onClose, triggerRef }: { items: readonly AgentHistoryItem[]; loading?: boolean; now?: Date; onSelect: (id: string) => void; onClose: () => void; triggerRef?: RefObject<HTMLButtonElement> }) {
  ...
  const stableNow = now ?? mountedNow;
    ...
    <aside ref={layer.ref as RefObject<HTMLElement>} aria-label="对话历史" className="agent-v6-history">
      ...
      {loading ? <div className="agent-v6-history-state">正在加载历史...</div> : items.length === 0 ? <div className="agent-v6-history-state">暂无对话历史</div> : (
        <div className="agent-v6-history-list">
          {Object.entries(groups).map(([date, entries]) => <section key={date}><h2>{dateLabel(date, stableNow)}</h2>{entries.map((item) => <button aria-current={item.selected ? "true" : undefined} aria-label={`${item.title}${item.summary ? `，${item.summary}` : ""}`} className={`agent-v6-history-item ${item.selected ? "is-selected" : ""}`} key={item.id} type="button" onClick={() => { onSelect(item.id); layer.dismissLayer(); }}><strong>{item.title}</strong>{item.summary ? <span>{item.summary}</span> : null}</button>)}</section>)}
        </div>
      )}
    </aside>
  );
