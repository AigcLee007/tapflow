...
export function NodeInputTray({ disabled = false, items, onFocusSource, onRemove, onReorder, onRetryPreview }: NodeInputTrayProps) {
  ...
  const [flyoutStyle, setFlyoutStyle] = useState<CSSProperties>({ position: "fixed", visibility: "hidden", zIndex: IMAGE_MENU_SURFACE_Z_INDEX });
  ...
  useLayoutEffect(() => {
    ...
    const positionFlyout = () => {
      ...
      if (flyoutPosition.current === key) return; flyoutPosition.current = key; setFlyoutStyle({ left, position: "fixed", top, visibility: "visible", zIndex: IMAGE_MENU_SURFACE_Z_INDEX });
    };
