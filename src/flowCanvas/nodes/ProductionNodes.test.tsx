...
describe('production suite nodes', () => {
  ...
  it('renders production node open actions with clean labels', () => {
    render(
      <StoryboardNodeComponent
        {...baseProps}
        data={{ kind: 'storyboard', title: '故事板', width: 360, height: 260, status: 'idle' }}
      />,
    );
    render(
      <Director3dNodeComponent
    ...
    expect(screen.getByRole('button', { name: '打开故事板' })).toBeTruthy();
  ...
  it('opens each production studio from node actions', () => {
    ...
    expect((listener.mock.calls[2][0] as CustomEvent).detail).toMatchObject({
      nodeId: 'video-node',
      studio: 'video_editor',
    });
